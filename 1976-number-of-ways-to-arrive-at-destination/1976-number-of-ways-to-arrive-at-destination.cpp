...
public: 
    ...
    int countPaths(int n, vector<vector<int>>& roads) {
        ...
        set<pair<long long, long long>> st; // pair{dist, src};
        st.insert({0, 0});
        dist[0] = 0;
        ...
        while(!st.empty())
        {
            ...
            for(auto i : adj[node])
            {
                ...
                {
                    dist[nnode] = d + wt;
                    st.insert({d + wt, nnode});
                    (ways[nnode] = ways[node]) %= mod;
                }
                else if(d + wt == dist[nnode])
                {
                    (ways[nnode] += 0LL + ways[node]) %= mod;
                }
            }
        }
...
//     Runtime: 197 ms, faster than 5.04% of C++ online submissions for Number of Ways to Arrive at Destination.
// Memory Usage: 54 MB, less than 5.09% of C++ online submissions for Number of Ways to Arrive at Destination.
